...
async function run() {
    ...
    try {
        ...
        if (keepRunning) {
            // expose the emulator to subsequent steps so they don't need to hardcode the port
            core.exportVariable('EMULATOR_PORT', `${port}`);
            core.exportVariable('ANDROID_SERIAL', `emulator-${port}`);
            // let the post step kill the emulator at the end of the job
            core.saveState('keepRunning', 'true');
            core.saveState('emulatorPort', `${port}`);
